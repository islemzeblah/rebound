...
# Global coin cache - fetched once per 24h
coin_cache = {
    "pairs": [],
    "last_fetch": None,
    "cache_hours": 24
...
class WebScanner:
    ...
    def get_all_usdt_pairs(self) -> List[str]:
        """Get all USDT spot trading pairs - cached for 24 hours."""
        global coin_cache
        
        # Check if cache is valid
        if coin_cache["pairs"] and coin_cache["last_fetch"]:
            time_since_fetch = datetime.now() - coin_cache["last_fetch"]
            if time_since_fetch.total_seconds() < coin_cache["cache_hours"] * 3600:
                print(f"Using cached pairs (last fetch: {coin_cache['last_fetch'].strftime('%H:%M %d/%m')})")
                return coin_cache["pairs"]
        
        # Fetch new pairs
        try:
            print("Fetching fresh trading pairs...")
            ...
            # Update cache
            coin_cache["pairs"] = pairs
            coin_cache["last_fetch"] = datetime.now()
            
            print(f"✓ Found {len(pairs)} USDT spot trading pairs (cached for {coin_cache['cache_hours']}h)")
            return pairs
            
        except Exception as e:
            print(f"Error fetching pairs: {e}")
            # Return cached pairs if available
            return coin_cache["pairs"] if coin_cache["pairs"] else []
...
def scan():
    ...
    try:
        ...
        max_results = scanner.max_results
        
        # Convert results to dictionaries for JSON response - LIMITED TO max_results
        results_dict = []
...
def cache_status():
    """Get coin cache status."""
    global coin_cache
    
    if coin_cache["last_fetch"]:
        time_since = datetime.now() - coin_cache["last_fetch"]
        hours_since = time_since.total_seconds() / 3600
        next_fetch = coin_cache["cache_hours"] - hours_since
        
        return jsonify({
            "cached": True,
            "pairs_count": len(coin_cache["pairs"]),
            "last_fetch": coin_cache["last_fetch"].strftime('%Y-%m-%d %H:%M:%S'),
            "hours_since": round(hours_since, 1),
            "next_fetch_in": round(next_fetch, 1) if next_fetch > 0 else 0,
            "cache_hours": coin_cache["cache_hours"]
        })
    else:
...
<head>
    ...
    <style>
        * {
        ...
        /* Color classes for percentages - with background colors and comments */
        .cell-red { /* >30% - Extreme */
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <form id="scanForm">
                <div class="config-grid">
                    ...
                    <!-- ADDED MAX RESULTS INPUT -->
                    <div class="config-item">
                        <label>Max Results</label>
                        <input type="number" name="max_results" value="100" min="1" max="1000" step="1">
                    </div>
                </div>
        ...
        <div class="status-bar hidden" id="statusBar">
            <div class="status-message">
                ...
                <span class="max-results-badge" id="maxResultsBadge"></span>
            </div>
    ...
    <div id="docsModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <div class="doc-grid">
                        ...
                        <div class="doc-card">
                            ...
                            <ul>
                                ...
                                <li><strong>Max Results:</strong> Number of results to display (default: 100)</li>
                            </ul>
                        </div>
                        <div class="doc-card">
                            <h5>Timeframes</h5>
                ...
                <!-- Coin Cache System -->
                <div class="modal-section">
                    <h3>💾 Coin Cache System</h3>
                    <p>The scanner caches the list of trading pairs for 24 hours to reduce API calls and speed up scans.</p>
                    <div class="example-box">
                        • Pairs are fetched once every 24 hours<br>
                        • Cache status shows when pairs were last fetched<br>
                        • Next fetch time is displayed in the cache bar<br>
                ...
                <!-- Max Results Feature -->
                <div class="modal-section">
                    <h3>📊 Max Results Feature</h3>
                    <p>The scanner limits displayed results to the number you specify in the <strong>Max Results</strong> field. This keeps the interface responsive and focused on top performers.</p>
                    <div class="example-box">
                        • Only top X coins (by your selected filter) are shown<br>
                        • Badge shows "Showing top X of Y"<br>
                        • Full dataset available in CSV/JSON downloads<br>
                        • Adjust the number in the configuration panel
                    </div>
                </div>
                ...
                <div class="modal-section">
                    ...
                    <ul style="color: #8b949e; margin-left: 20px;">
                        ...
                        <li><strong>Cache system</strong> makes repeated scans faster</li>
                        <li><strong>Max Results</strong> focuses on top performers - adjust as needed</li>
                    </ul>
                </div>
    ...
    <script>
        ...
        let maxResults = 100;  // Will be set from server
            ...
            try {
                ...
                if (data.cached) {
                    cacheStatus.innerHTML = `
                        <span>📦 Pairs cached: <span>${data.pairs_count}</span> coins</span>
                        <span>⏱️ Last fetch: <span>${data.last_fetch}</span></span>
                        <span>⏳ Next fetch in: <span>${data.next_fetch_in} hours</span></span>
                    `;
                } else {
                    cacheStatus.innerHTML = '📦 No cached pairs yet - will fetch on first scan';
                }
            } catch (error) {
        ...
        document.getElementById('scanForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Update status bar with max results info
                    const statusMessage = document.getElementById('statusMessage');
                    const maxResultsBadge = document.getElementById('maxResultsBadge');
                    
                    statusMessage.textContent = `Scan completed at ${data.scan_time} • Found ${data.total_found} rebounds`;
                    if (data.total_found > maxResults) {
                        maxResultsBadge.textContent = `Showing top ${maxResults} of ${data.total_found}`;
                        maxResultsBadge.classList.remove('hidden');
                    } else {
                        maxResultsBadge.classList.add('hidden');
                    }
        ...
        document.getElementById('resetBtn').addEventListener('click', function() {
            document.querySelectorAll('input[type="number"]').forEach(input => {
                ...
                if (input.name === 'max_results') input.value = '100';  // ADDED THIS
            });
        ...
        function getCellClass(value, is21d = false) {
            if (is21d) {
                if (value >= 42) return 'cell-purple';  /* >42% - 21d Special */
                if (value >= 35) return 'cell-blue';    /* 35-42% - 21d High */
                if (value >= 20) return 'cell-green';   /* 20-35% - 21d Moderate */
                if (value >= 10) return 'cell-yellow';  /* 10-20% - 21d Low */
                return 'cell-gray';                      /* <10% - Negative */
            } else {
                if (value >= 30) return 'cell-red';     /* >30% - Extreme */
                if (value >= 20) return 'cell-orange';  /* 20-30% - Very High */
                if (value >= 10) return 'cell-yellow';  /* 10-20% - High */
                if (value >= 5) return 'cell-green';    /* 5-10% - Moderate */
                if (value >= 0) return 'cell-blue';     /* 0-5% - Low */
                return 'cell-gray';                       /* Negative */
            }
        }

        function getVolumeCellClass(volumeStr) {
            const volume = parseFloat(volumeStr.replace(/[$,]/g, ''));
            if (volume >= 10000000) return 'cell-purple';  /* >$10M */
            if (volume >= 5000000) return 'cell-blue';     /* $5-10M */
            if (volume >= 2000000) return 'cell-green';    /* $2-5M */
            if (volume >= 1000000) return 'cell-yellow';   /* $1-2M */
            return 'cell-gray';                              /* <$1M */
        }
    ...
    print("💾 Coin Cache: Pairs fetched ONCE per 24 hours")
    print("🎨 Colorized Cells with percentage comments in CSS")
    print("🔍 Column Filtering Available")
    print("📉 21d Drawdown with Flags Added")
    print("📚 Full Documentation Available")
    print("📊 Max Results: Configurable in website")
    print("=" * 50)
    print("\nCoin Cache Features:")
    print("• Pairs fetched only once every 24 hours")
    print("• Cache status bar shows last fetch time")
    print("• Next fetch countdown displayed")
    print("• Reduces API calls and speeds up scans")
    print("=" * 50)
    print("\nMax Results Feature:")
    print("• Now configurable directly on the website!")
    print("• Adjust the number in the configuration panel")
    print("• Badge shows total found vs displayed")
    print("• Full data available in downloads")
    print("• Range: 1-1000 results")
    print("=" * 50)
    print("\nColor Comments Added:")
    print("• CSS classes now have inline comments showing percentages")
    print("• Red: >30% - Extreme")
    print("• Orange: 20-30% - Very High")
    print("• Yellow: 10-20% - High")
    print("• Green: 5-10% - Moderate")
    print("• Blue: 0-5% - Low")
    print("• Purple: >42% (21d special)")
